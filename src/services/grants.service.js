...
function createGrant({ patientId, granteeId, expiresAt }) {
  ...
  // Verificar que el grantee existe y es doctor
  const grantee = db.users.find((u) => u.id === granteeId);
  if (!grantee) {
    return { ok: false, status: 404, error: "Grantee not found" };
  }
  if (grantee.role !== "doctor") {
    return {
      ok: false,
      status: 400,
      error: "Only doctors can be granted access",
    };
  }
...
function revokeGrant({ patientId, grantId }) {
  ...
  if (grant.revoked_at) {
    return { ok: false, status: 400, error: "Grant already revoked" };
  }
